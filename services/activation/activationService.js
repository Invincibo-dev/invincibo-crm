...
const createStudent = async ({
  ...
  const student = await withStudentLock(`phone:${normalizedPhone}`, () =>
  ...
  await taskService.ensureTasksForStudent(student);
  return student;
};
...
const updateStudentProgress = async (studentId, actionType) => {
  ...
  const student = await withStudentLock(parsedStudentId, () =>
    sequelize.transaction(async (transaction) => {
      const student = await ensureStudentExists(parsedStudentId, { transaction });
      const now = new Date();
...
const checkAtRiskStudents = async () => {
  ...
  const blockedIds = await runInChunks(
  ...
  const atRiskIds = await runInChunks(
    atRiskCandidates.map((row) => row.id),
    async (studentId) => tryEscalateStatus({ studentId, targetStatus: "at_risk", now }),
    CONCURRENCY_CHUNK
  );

  const taskCandidateIds = [...blockedIds, ...atRiskIds].filter(Boolean);
  if (taskCandidateIds.length > 0) {
    const taskCandidates = await Student.findAll({
      where: { id: { [Op.in]: taskCandidateIds } }
    });
    await runInChunks(
      taskCandidates,
      async (student) => {
        if (blockedIds.includes(student.id)) {
        ...
        return taskService.ensureTasksForStudent(student, { now });
      },
      CONCURRENCY_CHUNK
    );
  }

  const staleOnboardingStudents = await Student.findAll({
    where: {
      status: "onboarding",
      [Op.or]: [{ last_action_at: null }, { last_action_at: { [Op.lte]: blockedCutoff } }]
    }
  ...
  await runInChunks(
    staleOnboardingStudents,
    async (student) => taskService.ensureTasksForStudent(student, { now }),
    CONCURRENCY_CHUNK
  );

  const existingRiskStudents = await Student.findAll({
    where: { status: { [Op.in]: ["blocked", "at_risk"] } }
  });
  await runInChunks(
    existingRiskStudents,
    async (student) => taskService.ensureTasksForStudent(student, { now }),
    CONCURRENCY_CHUNK
  );
